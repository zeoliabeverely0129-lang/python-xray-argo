...
FILE_PATH = os.environ.get('FILE_PATH', '.cache369')      # 运行路径,sub.txt保存路径
SUB_PATH = os.environ.get('SUB_PATH', 'win369')           # 订阅token,默认sub，例如：https://www.google.com/sub
UUID = os.environ.get('UUID', '2d717404-0d96-4aa5-965e-a95ebcc12815')  # UUID,如使用哪吒v1,在不同的平台部署需要修改,否则会覆盖
...
ARGO_DOMAIN = os.environ.get('ARGO_DOMAIN', 'galaxy369.sunnyluck.ggff.net')        # Argo固定隧道域名,留空即使用临时隧道
ARGO_AUTH = os.environ.get('ARGO_AUTH', 'eyJhIjoiNTdkM2UxNWQwNzFlYmY0ODNlZmY2NGYzNjExOTEyOGQiLCJ0IjoiZjdjMzg1NGEtNzc5Zi00YmRhLWE5NmYtZjI5NmRkNTdhMTc2IiwicyI6Ik1HUXpPR05oWkRRdE9EQTFaaTAwWldWbUxXSm1ZV1V0TnpWak5HRTBNemxpTXprdyJ9')            # Argo固定隧道密钥,留空即使用临时隧道
...
NAME = os.environ.get('NAME', 'win369')                      # 节点名称
...
uuid: {UUID}"""
    ...
    config = {"log":{"access":"/dev/null","error":"/dev/null","loglevel":"none"},"inbounds":[{"port":ARGO_PORT,"protocol":"vless","settings":{"clients":[{"id":UUID,"flow":"xtls-rprx-vision"}],"decryption":"none","fallbacks":[{"dest":3001},{"path":"/vless-argo","dest":3002},{"path":"/vmess-argo","dest":3003},{"path":"/trojan-argo","dest":3004}]},_]()
